...
server_ip = "127.0.0.1"
server_port = 10000
fragment_size = 1800  # In bytes
window_size = 20  # In frames (for SRP)
time_out = 50  # Default time out for packets (in seconds)
file_name = "unpickled_message_json_device0client0.json"
options = tuple()
args = list()
try:
    options, args = getopt.getopt(cli_args, "i:p:b:w:t:f:h:", ["server_ip =", "server_port =", "fragment_size =", "window_size =", "time_out =", "file_name =", "help"])
except Exception:
    print("Error Message ")
...
for index in range(len(options)):
    pointer = options[index]  # Get the tuple
    if pointer[0] == '--server_ip' or pointer[0] == '-i' or pointer[0] == '--server_ip ':
        server_ip = pointer[1]
        print("Set server IP to: " + str(server_ip))
    elif pointer[0] == '--server_port' or pointer[0] == '--server_port ' or pointer[0] == '-p':
        print("Set server port to: " + str(pointer[1]))
        server_port = int(pointer[1])
    elif pointer[0] == '--fragment_size' or pointer[0] == '--fragment_size ' or pointer[0] == '-b':  # 'b' is actually shorthand for bytes, which means the fragment size in bytes.
        print("Set fragment size to: " + str(pointer[1]))
        fragment_size = int(pointer[1])
    elif pointer[0] == '--window_size' or pointer[0] == '--window_size ' or pointer[0] == '-w':
        print("Set window size to: " + str(pointer[1]))
        window_size = int(pointer[1])
    elif pointer[0] == '--time_out' or pointer[0] == '--time_out ' or pointer[0] == '-t':
        print("Set timeout value to: " + str(pointer[1]))
        time_out = int(pointer[1])
    elif pointer[0] == '--file_name' or pointer[0] == '--file_name ' or pointer[0] == '-f':
        print("File name to send set to: ", str(pointer[1]))
        file_name = str(pointer[1])
    elif pointer[0] == '--help' or pointer[0] == '--help ' or pointer[0] == '-h':
        print("Help: ")
        print("-i<remote server ip here> or --server_ip <remote server ip here>" + ", Sets remote server IP")
        print("-p<remote server port here> or --server_port <remote server port here>, Sets remote server port")
        sys.exit(0)
...
# Display window and reset variables for subsequent windows
...
def process_final_window_packet():
    ...
    if server_response == "ACK_ALL":
        ...
        print("Packet Number #" + str(
            index_of_packet_not_received) + " was lost. Resending the lost packet")  # Displaying which packets to resend.
        seq_of_packet_not_received = int(
            index_of_packet_not_received) % window_size  # Taking remainder by window size so index number is converted to sequence number.
        UDP_socket.sendto(sender_data_buffer[int(seq_of_packet_not_received)],
                          (server_ip, server_port))  # Resending the lost packet.
        ...
        if time_out_tries < max_time_out_tries:
            time_out_tries += 1
            print("Request timed-out. Requesting acknowledgement again... (Tries left: %i/%i)" % (
                max_time_out_tries - time_out_tries, max_time_out_tries))
...
UDP_socket.sendto("CONN_REQ", (str(server_ip), int(server_port)))
