...
window_size = 30  # In frames (for SRP)
time_out = 4  # Default time out for packets (in seconds)
file_name = "unpickled_message_json_device0client0.json"
options = tuple()
args = list()
try:
    options, args = getopt.getopt(cli_args, "i:p:b:w:t:f:", ["server_ip =", "server_port =", "fragment_size =", "window_size =", "time_out =", "file_name ="])
except Exception:
    print("Error Message ")

for name, value in options:
    if name in ['-i', '--server_ip']:
        server_ip = value
    elif name in ['-p', '--server_port']:
        server_port = value
    elif name in ['-b', '--fragment_size']:
        fragment_size = value
    elif name in ['-w', '--window_size']:
        window_size = value
    elif name in ['-t', '--time_out']:
        time_out = value
    elif name in ['-f', '--file_name']:
        file_name = value
